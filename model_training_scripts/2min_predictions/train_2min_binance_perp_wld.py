...
MODEL_SAVE_DIR = "models/2min_binance_perp_wld"
BATCH_SIZE = 4
LEARNING_RATE = 1e-4
WARMUP_STEPS = 1000
EPOCHS = 50
...
logger = logging.getLogger(f"2min_binance_perp_wld_training")

# Check for CUDA availability
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
logger.info(f"Using device: {DEVICE}")
logger.info(f"Target: 2-minute predictions for {TARGET_EXCHANGE} WLD")
