...
TARGET_MINUTES = 1
TARGET_STEPS = 12  # 1 min * 60 sec/min / 5 sec/step
TARGET_PAIR = "WLD-USDT"
TARGET_EXCHANGE = "binance_perp"

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger(f"1min_binance_perp_wld_training")

# Check for CUDA availability
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
logger.info(f"Using device: {DEVICE}")
logger.info(f"Target: 1-minute predictions for {TARGET_EXCHANGE} WLD")
...
class TargetSpecificLOBForecaster(nn.Module):
    ...
    def forward(self, src, tgt):
        ...
        # Reshape for transformer (flatten features into sequence dimension)
        batch_size, context_len, _ = src.shape
        target_len = tgt.shape[1]
        
        # Flatten features into sequence: (batch, time*features, embed_dim)
        ...
        # Project to output
        output = self.output_layer(transformer_out)  # (batch, target_len*num_target_feat, 1)
        
        # Reshape back to target format: (batch, target_len, num_target_features)
...
def main():
    ...
    logger.info(f"Target: {TARGET_EXCHANGE} WLD")
    logger.info(f"Target features: {num_target_features} out of {len(embedding_metadata['columns'])}")
    logger.info(f"Prediction horizon: 1 minutes (12 steps)")
