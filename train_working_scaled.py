#!/usr/bin/env python3
"""
Scaled Multi-GPU Training: Uses all GPUs and much more memory for faster training
Based on working adapted script but optimized for high-performance multi-GPU setup
"""
...
# --- SCALED CONFIGURATION FOR MULTI-GPU ---
FINAL_DATA_DIR = "data/final_attention"
MODEL_SAVE_DIR = "models/working_scaled_multigpu"
BATCH_SIZE = 32  # Much larger batch size for multi-GPU (was 4)
LEARNING_RATE = 1e-4
WARMUP_STEPS = 1000
EPOCHS = 50
PATIENCE = 8
EMBED_DIM = 256  # Larger embedding (was 128)
NUM_HEADS = 8    # 256/8 = 32 (divisible)
NUM_ENCODER_LAYERS = 6  # Deeper model
NUM_DECODER_LAYERS = 4  # Deeper model
DROPOUT = 0.1
REG_WEIGHT = 0.01
NUM_WORKERS = 16  # More workers for multi-GPU

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger("working_scaled_multigpu")
...
class WorkingScaledDataset(Dataset):
    """Dataset for scaled multi-GPU training."""
...
class ScaledCrossMarketEmbedding(nn.Module):
    """Scaled compound embedding for all market features."""
...
class ScaledBinancePerpEmbedding(nn.Module):
    """Scaled embedding for Binance perp output features."""
...
class ScaledPositionalEncoding(nn.Module):
    """Positional encoding optimized for large sequences."""
...
class ScaledMultiGPUForecaster(nn.Module):
    """Scaled model optimized for multi-GPU training."""
    def __init__(self, embedding_metadata, target_feature_indices, embed_dim, num_heads, 
        ...
        # Use more efficient transformer layers
...
def main():
    ...
    logger.info("🚀 Scaled Multi-GPU Training Started!")
    ...
    # Create data loaders with more workers for multi-GPU
    ...
    # --- Model Creation ---
    logger.info("Initializing scaled multi-GPU model...")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_loss < best_val_loss:
            ...
            torch.save(checkpoint, os.path.join(MODEL_SAVE_DIR, 'best_model_scaled.pt'))
    ...
    logger.info("🎉 Scaled Multi-GPU Training completed!")
