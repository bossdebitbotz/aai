#!/usr/bin/env python3
"""
Diagnose Target Prediction - Test ALL vs WLD-only predictions

The validation loss mismatch suggests the model was trained differently.
This tests whether it predicts all 240 features vs just 20 WLD features.
"""
...
class TestDataset(Dataset):
    """Dataset for testing different target configurations."""
    def __init__(self, file_path, target_feature_indices=None):
        with np.load(file_path, allow_pickle=True) as data:
            ...
            if target_feature_indices is not None:
                # Use only specific features (WLD-only)
                self.target = self.target_full[:, :, target_feature_indices]
            else:
                # Use all features (240)
...
def get_wld_indices(embedding_metadata):
...
def test_target_configuration(target_indices, config_name):
    """Test a specific target configuration."""
    print(f"\n🧪 Testing {config_name}...")
    ...
    num_target_features = len(target_indices) if target_indices else 240
    
    print(f"  - Target features: {num_target_features}")
    print(f"  - Target indices: {target_indices[:5] if target_indices else 'ALL'}...")
    
    # Create model
    model = ScaledMultiGPUForecaster(
        embedding_metadata=embedding_metadata,
        target_feature_indices=target_indices or list(range(240)),
    ...
    difference = abs(saved_val_loss - computed_val_loss)
    
    print(f"  - Computed validation loss: {computed_val_loss:.6f}")
    print(f"  - Difference from saved:    {difference:.6f}")
    
    return {
        'config_name': config_name,
        'num_features': num_target_features,
        'computed_loss': computed_val_loss,
        'difference': difference,
        'match_quality': 'EXCELLENT' if difference < 0.001 else 'GOOD' if difference < 0.01 else 'POOR'
    }

def main():
    """Test different target configurations."""
    print("🔍 DIAGNOSING TARGET PREDICTION CONFIGURATION")
    print("=" * 60)
    
    # Load metadata for WLD indices
    with open(os.path.join(FINAL_DATA_DIR, 'embedding_metadata.json'), 'r') as f:
        embedding_metadata = json.load(f)
    
    wld_indices = get_wld_indices(embedding_metadata)
    saved_val_loss = torch.load(MODEL_PATH, map_location='cpu')['val_loss']
    
    print(f"🎯 Target: Match saved validation loss of {saved_val_loss:.6f}")
    
    results = []
    
    # Test 1: All 240 features
    result1 = test_target_configuration(None, "ALL 240 FEATURES")
    if result1:
        results.append(result1)
    
    # Test 2: WLD-only (20 features)  
    result2 = test_target_configuration(wld_indices, "WLD-ONLY (20 features)")
    if result2:
    ...
    best_match = min(results, key=lambda x: x['difference']) if results else None
    
    for result in results:
        status = "🎯 WINNER!" if result == best_match else "❌ NO MATCH"
        print(f"{status} {result['config_name']}")
        print(f"    - Features: {result['num_features']}")
        print(f"    - Loss: {result['computed_loss']:.6f}")
        print(f"    - Difference: {result['difference']:.6f} ({result['match_quality']})")
    
    if best_match:
        print(f"\n✅ CONCLUSION:")
        print(f"   The model was trained to predict: {best_match['config_name']}")
        print(f"   Use this configuration for accurate backtesting!")
    else:
        print(f"\n❌ No configuration matched! Architecture issue detected.")
