...
MODEL_SAVE_DIR = "models/5min_binance_perp_sol"
BATCH_SIZE = 4
LEARNING_RATE = 1e-4
WARMUP_STEPS = 1000
EPOCHS = 50
...
logger = logging.getLogger(f"5min_binance_perp_sol_training")

# Check for CUDA availability
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
logger.info(f"Using device: {DEVICE}")
logger.info(f"Target: 5-minute predictions for {TARGET_EXCHANGE} SOL")
