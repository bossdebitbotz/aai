...
MODEL_SAVE_DIR = "models/1min_binance_perp_btc"
BATCH_SIZE = 4
LEARNING_RATE = 1e-4
WARMUP_STEPS = 1000
EPOCHS = 50
...
logger = logging.getLogger(f"1min_binance_perp_btc_training")

# Check for CUDA availability
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
logger.info(f"Using device: {DEVICE}")
logger.info(f"Target: 1-minute predictions for {TARGET_EXCHANGE} BTC")
