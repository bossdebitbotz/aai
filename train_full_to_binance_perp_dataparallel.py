#!/usr/bin/env python3
"""
Full Market Context to Binance Perpetual Prediction Model - DataParallel (4 GPU)

Simpler 4-GPU version using DataParallel instead of DistributedDataParallel.
More reliable but slightly less efficient than DDP.
"""
...
MODEL_SAVE_DIR = "models/full_to_binance_perp_dataparallel"
BATCH_SIZE = 32   # Large batch size for 4 GPUs
LEARNING_RATE = 1e-4
WARMUP_STEPS = 1000
EPOCHS = 130      # Stop at epoch 130 (was performing well)
...
logger = logging.getLogger("full_to_binance_perp_dataparallel")
...
def main():
    """Main training function using DataParallel"""
    logger.info("🚀 Full Market → Binance Perp DataParallel Training Started!")
    ...
    for epoch in range(1, EPOCHS + 1):
        ...
        if val_metrics['total_loss'] < best_val_loss:
            ...
            torch.save(checkpoint, os.path.join(MODEL_SAVE_DIR, 'best_model_dataparallel.pt'))
