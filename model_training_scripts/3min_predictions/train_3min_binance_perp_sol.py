...
MODEL_SAVE_DIR = "models/3min_binance_perp_sol"
BATCH_SIZE = 4
LEARNING_RATE = 1e-4
WARMUP_STEPS = 1000
EPOCHS = 50
...
logger = logging.getLogger(f"3min_binance_perp_sol_training")

# Check for CUDA availability
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
logger.info(f"Using device: {DEVICE}")
logger.info(f"Target: 3-minute predictions for {TARGET_EXCHANGE} SOL")
...
def main():
    ...
    logger.info(f"Target: {TARGET_EXCHANGE} SOL")
