#!/usr/bin/env python3
"""
Binance Perpetual-Only LOB Forecasting Model Training

This script trains a specialized attention-based LOB forecasting model
using only Binance perpetual features (80 features vs 240 in the general model).

Optimizations:
1. Specialized for perpetual market characteristics
2. Reduced feature space (80 vs 240)
3. H100 GPU optimization
4. Enhanced structural regularizer for perp-specific constraints
"""
...
# --- Configuration ---
FINAL_DATA_DIR = "data/final_binance_perp"
MODEL_SAVE_DIR = "models/binance_perp_specialized"
BATCH_SIZE = 16  # Increased for H100
LEARNING_RATE = 1e-4
WARMUP_STEPS = 500  # Reduced for smaller dataset
EPOCHS = 100
PATIENCE = 10
EMBED_DIM = 80  # Matches feature count, divisible by 4 heads
NUM_HEADS = 4  # Increased for better attention
NUM_ENCODER_LAYERS = 4  # Deeper for specialized learning
NUM_DECODER_LAYERS = 4
DROPOUT = 0.15  # Slightly higher for regularization
REG_WEIGHT = 0.015  # Slightly higher structural regularizer
NUM_WORKERS = 8  # H100 optimized

# Set up logging
logging.basicConfig(level=logging.INFO, format='%(asctime)s - %(name)s - %(levelname)s - %(message)s')
logger = logging.getLogger("binance_perp_training")
...
class BinancePerpLOBDataset(Dataset):
    """Dataset for Binance perpetual LOB data."""
    ...
    def __getitem__(self, idx):
        with np.load(self.file_path) as data:
            x = data['x'][idx]  # (context_len, num_features)
            y = data['y'][idx]  # (target_len, num_features)
            
        return torch.from_numpy(x).float(), torch.from_numpy(y).float()

# --- 2. Embedding Layers ---

class BinancePerpCompoundEmbedding(nn.Module):
    """Specialized compound embedding for Binance perpetual features only."""
    def __init__(self, embedding_metadata, embed_dim):
        super().__init__()
        self.embed_dim = embed_dim
        self.metadata = embedding_metadata
        
        # Since we only have Binance perp, simplify the embedding structure
        unique_attrs = embedding_metadata['unique_attributes']
        
        # Calculate embedding sizes - we only need 4 types now (no exchange needed)
        ...
        self.pair_embedding = nn.Embedding(len(unique_attrs['trading_pairs']), attr_embed_dim)
    ...
    def forward(self, num_features):
        """Create embeddings for all Binance perp features."""
        embeddings = []
...
class BinancePerpLOBForecaster(nn.Module):
    """Specialized LOB forecaster for Binance perpetuals."""
    def __init__(self, embedding_metadata, embed_dim, num_heads, 
                 num_encoder_layers, num_decoder_layers, dropout, target_len):
        super().__init__()
        self.embedding_metadata = embedding_metadata
        self.num_features = embedding_metadata['num_features']
        self.target_len = target_len
        self.embed_dim = embed_dim

        # --- Embedding Layers ---
        self.value_projection = nn.Linear(1, embed_dim)
        self.compound_embedding = BinancePerpCompoundEmbedding(embedding_metadata, embed_dim)
        self.positional_encoding = PositionalEncoding(embed_dim, dropout)

        # --- Enhanced Transformer for Perpetuals ---
        encoder_layer = nn.TransformerEncoderLayer(
            d_model=embed_dim,
            nhead=num_heads,
            dim_feedforward=embed_dim * 4,
            dropout=dropout,
            activation='gelu',  # Better activation for financial data
            batch_first=True
        )
...
class BinancePerpStructuralLoss(nn.Module):
    """Enhanced structural regularizer for Binance perpetuals."""
    def __init__(self, embedding_metadata):
        super().__init__()
        self.metadata = embedding_metadata
        
        # Organize price indices by trading pair
        self.price_indices_by_pair = {}
        
        for col_name, col_info in embedding_metadata['column_mapping'].items():
            if col_info['feature_type'] == 'price':
                ...
                self.price_indices_by_pair[pair][order_type][level] = col_info['column_idx']

    def forward(self, predictions):
        """Compute enhanced structural loss for perpetuals."""
        ...
        for pair, price_indices in self.price_indices_by_pair.items():
            ...
            max_level = min(max(bid_cols.keys()), max(ask_cols.keys()))
...
def main():
    ...
    logger.info("Starting Binance Perpetual LOB Model Training...")
    logger.info("=" * 60)
    ...
    logger.info(f"Features: {embedding_metadata['num_features']} (Binance perp only)")
    logger.info(f"Trading pairs: {', '.join(embedding_metadata['unique_attributes']['trading_pairs'])}")
    
    # Create datasets
    train_dataset = BinancePerpLOBDataset(os.path.join(FINAL_DATA_DIR, 'train.npz'))
    val_dataset = BinancePerpLOBDataset(os.path.join(FINAL_DATA_DIR, 'validation.npz'))
    ...
    # Initialize model
    logger.info("Initializing Binance perpetual model...")
    model = BinancePerpLOBForecaster(
    ...
    struct_loss_fn = BinancePerpStructuralLoss(embedding_metadata).to(DEVICE)
    ...
    logger.info("=" * 60)
    logger.info("BINANCE PERPETUAL MODEL TRAINING COMPLETE")
    logger.info("=" * 60)
