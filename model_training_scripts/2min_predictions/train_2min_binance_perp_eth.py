...
MODEL_SAVE_DIR = "models/2min_binance_perp_eth"
BATCH_SIZE = 4
LEARNING_RATE = 1e-4
WARMUP_STEPS = 1000
EPOCHS = 50
...
logger = logging.getLogger(f"2min_binance_perp_eth_training")

# Check for CUDA availability
DEVICE = torch.device("cuda" if torch.cuda.is_available() else "cpu")
logger.info(f"Using device: {DEVICE}")
logger.info(f"Target: 2-minute predictions for {TARGET_EXCHANGE} ETH")
...
def main():
    ...
    logger.info(f"Target: {TARGET_EXCHANGE} ETH")
    logger.info(f"Target features: {num_target_features} out of {len(embedding_metadata['columns'])}")
    logger.info(f"Prediction horizon: 2 minutes (24 steps)")
